...
impl Palette {
    ...
    pub fn insert_constant(&mut self, key: &str, val: Color) {
        let _ = self.selector_to_colors.insert(key.to_string(), ColorByMode::Constant(val));
    }

    pub fn insert_by_mode(&mut self, key: &str, dark: Color, light: Color) {
        let _ = self.selector_to_colors.insert(key.to_string(), ColorByMode::ModeBased {
            dark,
            light,
        });
    }
        ...
        where String : Borrow<Q>
    {
        ...
        match by_mode {
            ColorByMode::Constant(c) => {
                Some(c)
            }
            ColorByMode::ModeBased { dark, light } => {
                match mode.clone().detect() {
                    ThemeMode::Dark => {
                        Some(dark)
                    }
                    ThemeMode::Light => {
